...
useEffect(()=>{
axios.get(`http://localhost:5000/api/products?page=${pageNo+1}`).then((res)=>{
...
<Container> 
  ...
  <Box  sx={{marginLeft:'auto'}}>
     ...
     setPageNo(pageNo+1)
        ...
        >  <ArrowForward sx={{color:'blue',fontSize:'35px'}} /> </IconButton>  
...
):  products.length===6 && pageNo >=2 && additionalProducts.length!==0 ? (
<>
 <Box  sx={{marginLeft:'auto',display:'flex'}}>
     ...
     setPageNo(pageNo-1)
        ...
        >  <ArrowBack sx={{color:'blue',fontSize:'35px'}} /> </IconButton>  
